...
DEFAULT_SETTINGS: dict[str, tuple[str, str]] = {
    ...
    # 数据备份（v2.9.13+）：默认开启周期备份，本地与外挂目录均默认保留 30 份
    "backup_dir": (
        "/app/data/backups",
        "本地备份根目录（容器内绝对路径）；改了之后下个周期生效",
    ),
    "backup_remote_dir": (
        "",
        "外挂备份目录（空字符串=关闭；典型为宿主/NFS/SMB 挂载点）。为空则只保留本地",
    ),
    "backup_keep_local": (
        "30",
        "本地保留份数（按修改时间裁剪，0=不裁剪）",
    ),
    "backup_keep_remote": (
        "30",
        "外挂保留份数（按修改时间裁剪，0=不裁剪）",
    ),
    "backup_interval_hours": (
        "24",
        "自动备份周期（小时；0=关闭自动，只保留手动与启动期）",
    ),
    "backup_at_boot": (
        "0",
        "容器启动时是否立即做一次（1=是）。新部署或久未启动时建议打开",
    ),
}
